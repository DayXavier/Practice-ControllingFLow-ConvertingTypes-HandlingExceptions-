...
#region Working with single-dimensional arrys
...
// Allternatice syntax for creating and initializing an array.
...
WriteLine($"2nd dimension, lower bound: {grid1.GetUpperBound(1)}");

for (int row = 0; row <= grid1.GetUpperBound(0); row++)
{
    for (int col = 0; col <= grid1.GetUpperBound(1); col++)
    {
        WriteLine($"Row {row}, Column {col}: {grid1[row, col] ?? "No Value"}");
    }
}
...
static string CheckSwitch(int[] values) => values switch
{
    ...
    // Use Alt + up or down arrow to move statments.
};
